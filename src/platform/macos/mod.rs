...
use objc2_app_kit::{
    ...
    NSWindow, NSWindowStyleMask, NSWindowTabbingMode, NSWorkspace,
};
...
impl MacosWindowFeature {
    pub fn from_winit_window(window: &Window, settings: Arc<Settings>) -> Self {
        ...
        let macos_window_feature = MacosWindowFeature {
            ...
            force_click_url_preview: settings.get::<WindowSettings>().force_click_url_preview,
        };
    ...
    pub fn set_force_click_url_preview(&mut self, enabled: bool) {
        self.force_click_url_preview = enabled;
    }
    ...
    pub fn handle_force_click_target(
        ...
        let handled = match kind {
            ForceClickKind::Url => {
                if self.force_click_url_preview {
                    self.preview_url(entity)
                } else {
                    self.open_url(entity)
                }
            }
    ...
    pub fn handle_settings_changed(&mut self, changed_setting: WindowSettingsChanged) {
        match changed_setting {
            ...
            WindowSettingsChanged::ForceClickUrlPreview(preview) => {
                self.force_click_url_preview = preview;
            }
            _ => {}
        }
    }
