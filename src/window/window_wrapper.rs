    ...
    pub fn handle_window_command(&mut self, command: WindowCommand) {
        ...
        match command {
            ...
            WindowCommand::TouchpadPressure {
                ...
                let macos_feature = self.macos_feature_mut();
                macos_feature.handle_force_click_target(
                    &entity,
                    kind,
                    point,
                    guifont,
    ...
    pub fn handle_window_settings_changed(&mut self, changed_setting: WindowSettingsChanged) {
        ...
        match changed_setting {
            ...
            WindowSettingsChanged::ForceClickUrlPreview(preview) => {
                if let Some(feature) = self.macos_feature.as_mut() {
                    feature.set_force_click_url_preview(preview);
                }
            }
            _ => {}
        }
